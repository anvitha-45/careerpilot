  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">Job Explorer & Semantic Matching</h1>
        ...
        <button
          ...
          className="inline-flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white text-xs sm:text-sm font-semibold shadow-lg shadow-brand-500/20 transition self-start"
        >
      ...
      <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col md:flex-row items-stretch md:items-center gap-3 sm:gap-4">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 focus:border-brand-500 rounded-xl py-2.5 pl-10 pr-4 text-xs sm:text-sm text-white focus:outline-none"
          />
        </div>

        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <div className="flex-1 sm:flex-initial flex items-center space-x-2">
            <span className="text-xs sm:text-sm text-slate-400 font-medium shrink-0">Portal:</span>
            <select
              value={portal}
              onChange={(e) => setPortal(e.target.value)}
              className="flex-1 sm:flex-initial bg-slate-950 border border-slate-800 text-xs sm:text-sm text-white rounded-lg py-2 px-3 focus:outline-none"
            >
          ...
          <div className="flex-1 sm:flex-initial flex items-center space-x-2">
            <span className="text-xs sm:text-sm text-slate-400 font-medium shrink-0">Min Match:</span>
            <select
              value={minScore}
            ...
            >
              <div className="space-y-3 flex-1 min-w-0 w-full">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="text-base sm:text-lg font-bold text-white">{item.job.title}</h3>
                  ...
                  {item.status && item.status !== 'DISCOVERED' && (
                    <span className="text-xs px-2 py-0.5 rounded font-bold uppercase bg-brand-500/20 text-brand-300 border border-brand-500/30">
                      {item.status}
                    </span>
                  )}
                </div>
                ...
                <div className="text-xs text-brand-400/90 font-medium bg-brand-950/30 p-2.5 rounded-lg border border-brand-900/40">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full p-4 sm:p-6 shadow-2xl space-y-4 my-auto max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={handleImportJob} className="space-y-4">
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-slate-300 mb-1">Role Title</label>
                <input
                  type="text"
                  required
                  value={newTitle}
                  ...
                  className="w-full bg-slate-950 border border-slate-800 focus:border-brand-500 rounded-xl p-2.5 text-xs sm:text-sm text-white focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-xs sm:text-sm font-semibold text-slate-300 mb-1">Company Name</label>
                <input
                  type="text"
                  required
                  value={newCompany}
              ...
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-slate-300 mb-1">Portal / Source</label>
                <select
                  value={newPortal}
                  onChange={(e) => setNewPortal(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 text-xs sm:text-sm text-white rounded-xl p-2.5 focus:outline-none"
                >
                  <option value="LinkedIn">LinkedIn</option>
                  <option value="Naukri">Naukri</option>
                  <option value="Greenhouse">Greenhouse ATS</option>
              ...
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-slate-300 mb-1">Job Description Text</label>
                <textarea
                  required
                  rows={6}
                  ...
                  className="w-full bg-slate-950 border border-slate-800 focus:border-brand-500 rounded-xl p-2.5 text-xs sm:text-sm text-white focus:outline-none resize-none"
                />
              </div>
              ...
              <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2.5 pt-2">
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-750 text-slate-300 text-xs sm:text-sm font-semibold text-center transition"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={importing}
                  className="px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white text-xs sm:text-sm font-semibold shadow-md shadow-brand-500/20 transition text-center"
                >
